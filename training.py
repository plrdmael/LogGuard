...
def process_data_from_csv_file(path_to_file, output_score_file="Ressources/Result/anomalies_score_from_csv.csv", output_anomaly="Ressources/Result/anomalies_from_csv.csv"):
    ...
    data, encoders = encode_categorical_column_from_csv(data, list(categorical_cols))
    print(data["URL"])
    ...
    model = IsolationForest(contamination=0.0007,  max_samples=150)
    ...
    with open('iso_forest_260k.pkl', 'wb') as f:
        pickle.dump(model, f)
...
process_data_from_csv_file(path_to_file="Ressources/Normalized/dataNorm260k.csv",output_score_file="Ressources/Result/training_score.csv",output_anomaly="Ressources/Result/training_anomalie.csv")
